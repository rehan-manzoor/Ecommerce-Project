  ...
  return (
    <>
      ...
      <div className="management-list">
        {orders.map((order) => (
          <article className="management-card panel" key={order._id}>
            <div>
              ...
              {order.vendorOrders?.map((group) => (
                <div className="summary-row" key={group._id}>
                  <span>
                    {group.vendor?.storeName || "Vendor"} · {group.status} · {money(group.subtotal)}
                    {group.trackingNumber && ` · ${group.carrier}: ${group.trackingNumber}`}
                  </span>

                  <select
                    value={group.status}
                    onChange={(event) => onStatusChange(order, group, event.target.value)}
                  >
                    <option value={group.status}>{group.status}</option>

                    {ORDER_STATUSES.filter((status) => status !== group.status).map((status) => (
