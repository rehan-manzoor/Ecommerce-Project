...
export const addGuestItem = (product, quantity = 1, variantId = null) => {
  ...
  const found = items.find((item) => item.product._id === product._id && (item.variantId || null) === variantId);
  const stock = variantId ? product.variants?.find((variant) => variant._id === variantId)?.stock || 0 : product.stock;
