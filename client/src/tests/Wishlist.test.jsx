...
test('saved products load and can be removed', async () => {
  api.get.mockResolvedValue({ data: { data: [{ _id: 'product-1', name: 'Blue bag', price: 15, vendor: { storeName: 'Maker' } }] } });
...
test('moving an item to cart sends server request before removing it', async () => {
  api.get.mockResolvedValue({ data: { data: [{ _id: 'product-1', name: 'Blue bag', price: 15 }] } });
  api.post.mockResolvedValue({});
  api.delete.mockResolvedValue({ data: { data: [] } });
  render(<MemoryRouter><Wishlist /></MemoryRouter>);
  await screen.findByText('Blue bag');
  screen.getByRole('button', { name: 'Move to cart' }).click();
  await waitFor(() => expect(api.post).toHaveBeenCalledWith('/cart/add', { productId: 'product-1', quantity: 1 }));
});
