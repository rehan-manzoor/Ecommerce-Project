...
const previous = {
  cart: Cart.findOne,
  shipping: ShippingMethod.findOne,
  exists: ShippingMethod.exists,
};
after(() => {
  Cart.findOne = previous.cart;
  ShippingMethod.findOne = previous.shipping;
  ShippingMethod.exists = previous.exists;
});
test("variant selection rejects missing options and uses variant inventory", () => {
  const product = {
    ...
    variants: {
      length: 1,
      id: (id) => (id === "chosen" ? { _id: "chosen", price: 25, stock: 2 } : null),
    },
  };
...
test("checkout calculates price, shipping and tax from server records", async () => {
  ...
  const product = {
    ...
    variants: {
      length: 1,
      id: (id) =>
        String(id) === String(variantId) ? { _id: variantId, price: 25, stock: 3 } : null,
    },
  };
  Cart.findOne = () => ({
    populate: async () => ({ items: [{ product, variantId, quantity: 2 }] }),
  });
...
test("checkout refuses variant overselling", async () => {
  ...
  const product = {
    ...
    status: "active",
    approvalStatus: "approved",
    variants: { length: 1, id: () => ({ _id: variantId, price: 25, stock: 1 }) },
  };
  Cart.findOne = () => ({
    populate: async () => ({ items: [{ product, variantId, quantity: 2 }] }),
  });
