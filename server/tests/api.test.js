...
const [{ default: app }, { default: User }, { default: Vendor }, { default: Order }] = await Promise.all([import('../app.js'), import('../models/User.js'), import('../models/Vendor.js'), import('../models/Order.js')]);
...
before(() => {
  User.findById = (id) => ({ select: async () => ({ _id: { toString: () => String(id) }, email: 'test@example.com', role: String(id) === vendorId ? 'vendor' : 'customer', isBlocked: false }) });
  Vendor.findOne = async () => ({ _id: { toString: () => storeId }, status: 'approved' });
  Order.find = (filter) => {
    assert.equal(String(filter['vendorOrders.vendor']), storeId);
    return { populate: () => ({ sort: async () => [{ _id: 'order-1', user: { name: 'Buyer' }, totalAmount: 1000, items: [ { vendor: storeId, name: 'Ours' }, { vendor: strangerStore, name: 'Theirs' } ], vendorOrders: [ { vendor: { _id: storeId }, subtotal: 25, items: [{ name: 'Ours' }], status: 'confirmed' }, { vendor: { _id: strangerStore }, subtotal: 975, items: [{ name: 'Theirs' }], status: 'shipped' } ] }] }) };
  };
});
after(() => { User.findById = originals.user; Vendor.findOne = originals.vendor; Order.find = originals.order; });
...
test('protected admin route rejects missing and customer tokens', async () => {
  ...
  assert.equal((await request(app).get('/api/admin/orders').set('Authorization', `Bearer ${token(customerId)}`)).status, 403);
});
test('vendor route returns only the authorized seller group', async () => {
  const response = await request(app).get('/api/orders/vendor/mine').set('Authorization', `Bearer ${token(vendorId)}`);
...
test('product routes reject unauthorized creation', async () => {
  ...
  assert.equal((await request(app).post('/api/products').set('Authorization', `Bearer ${token(customerId)}`).send({ name: 'test' })).status, 403);
});
