...
export const getCartPricing = async (userId, couponCode = "", shippingMethodId = null) => {
  ...
  const items = cart.items.map((entry) => {
    ...
    if (!Number.isInteger(entry.quantity) || entry.quantity < 1 || choice.stock < entry.quantity)
  ...
  } else if (await ShippingMethod.exists({ active: true }))
    throw new Error("Select a shipping method");
