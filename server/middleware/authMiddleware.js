...
export const protect = async (req, res, next) => {
  try {
    ...
    if (!authHeader?.startsWith("Bearer ")) {
      return res.status(401).json({ success: false, message: "Authentication required", data: null, error: null });
    }
    ...
    if (!user || user.isBlocked) {
      return res.status(401).json({ success: false, message: "Account is unavailable", data: null, error: null });
    }
