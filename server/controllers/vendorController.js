...
export const createVendor = async (req, res, next) => {
  try {
    ...
    if (!storeName?.trim())
      return res
        .status(400)
        .json({ success: false, message: "Store name is required", data: null, error: null });
    if (await Vendor.findOne({ user: req.user.userId }))
...
export const updateMyVendor = async (req, res, next) => {
  try {
    ...
    const vendor = await Vendor.findOneAndUpdate({ user: req.user.userId }, updates, {
      new: true,
      runValidators: true,
    });
    if (!vendor)
...
export const updateVendorStatus = async (req, res, next) => {
  try {
    const status =
      req.body.status ||
      (req.body.approved === true ? "approved" : req.body.approved === false ? "rejected" : null);
    if (!["pending", "approved", "rejected", "suspended"].includes(status)) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid vendor status", data: null, error: null });
    }
    ...
    if (!vendor)
      return res
        .status(404)
        .json({ success: false, message: "Vendor not found", data: null, error: null });
    ...
    await Notification.create({
      ...
      message: vendor.rejectionReason || `Your store is ${status}`,
      link: status === "approved" ? "/vendor" : "/become-vendor",
    }).catch(() => {});
...
export const getPublicVendor = async (req, res, next) => {
  try {
    const vendor = await Vendor.findOne({ storeSlug: req.params.slug, status: "approved" }).select(
      "storeName storeSlug description logo bannerImage"
    );
    if (!vendor)
      return res
        .status(404)
        .json({ success: false, message: "Store not found", data: null, error: null });
