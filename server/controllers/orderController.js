...
const populate = [
  ...
  { path: "items.vendor", select: "storeName storeSlug" },
  { path: "vendorOrders.vendor", select: "storeName storeSlug" },
];
...
const reserve = async (items) => {
  ...
  try {
    for (const item of items) {
      ...
      if (item.variantId)
        filter.variants = { $elemMatch: { _id: item.variantId, stock: { $gte: item.quantity } } };
...
const restore = (items) =>
  Promise.all(
    items.map((item) =>
      Product.updateOne(
        { _id: item.product, ...(item.variantId ? { "variants._id": item.variantId } : {}) },
        { $inc: { [item.variantId ? "variants.$.stock" : "stock"]: item.quantity } }
      )
    )
  );
const safeAddress = (body) => {
  ...
  if (
    !fields.every(
      (field) =>
        typeof body?.[field] === "string" && body[field].trim().length && body[field].length <= 160
    )
  )
    throw new Error("Complete shipping address is required");
  return Object.fromEntries(
    [...fields, "state"].map((field) => [field, String(body[field] || "").trim()])
  );
};
...
export const createOrder = async (req, res) => {
  try {
    ...
    if (
      ...
      return fail(res, 400, "Payment has not been verified");
    }

    payment.status = "succeeded";
    await payment.save();

    const order = await finalizePaidOrder(payment);

    return res.status(201).json({
      success: true,
      message: "Order created",
      data: await Order.findById(order._id).populate(populate),
    });
  } catch (error) {
    console.error("Order creation failed:", error);

    return fail(res, 400, "Unable to create order");
  }
};
export const getMyOrders = async (req, res) =>
  res.json({
    success: true,
    data: await Order.find({ user: req.user.userId }).populate(populate).sort({ createdAt: -1 }),
  });
export const getOrderById = async (req, res) => {
  const order = mongoose.isValidObjectId(req.params.id)
    ? await Order.findById(req.params.id).populate(populate)
    : null;
  if (!order) return fail(res, 404, "Order not found");
  if (req.user.role === "admin" || String(order.user._id) === req.user.userId)
  ...
  const group = order.vendorOrders.find(
    (entry) => String(entry.vendor?._id || entry.vendor) === String(vendor?._id)
  );
...
export const getAllOrders = async (req, res) =>
  res.json({
    success: true,
    data: await Order.find(req.query.status ? { status: req.query.status } : {})
      .populate(populate)
      .sort({ createdAt: -1 }),
  });
...
const transition = (group, status) =>
  ({
    ...
    returned: ["refunded"],
  })[group.status]?.includes(status);
export const updateVendorOrderStatus = async (req, res) => {
  const vendor = await Vendor.findOne({ user: req.user.userId, status: "approved" });
  const order =
    vendor && mongoose.isValidObjectId(req.params.id)
      ? await Order.findOne({ _id: req.params.id, "vendorOrders.vendor": vendor._id })
      : null;
  ...
  if (!["processing", "shipped"].includes(req.body.status) || !transition(group, req.body.status))
    return fail(res, 400, "Invalid vendor status transition");
  if (
    req.body.status === "shipped" &&
    (!String(req.body.carrier || "").trim() || !String(req.body.trackingNumber || "").trim())
  )
    return fail(res, 400, "Carrier and tracking number are required");
  group.status = req.body.status;
  if (group.status === "shipped") {
    group.carrier = String(req.body.carrier).slice(0, 80);
    group.trackingNumber = String(req.body.trackingNumber).slice(0, 100);
  ...
  order.statusHistory.push({
    status: order.status,
    note: `${vendor.storeName} updated fulfillment`,
  });
  await order.save();
  await notify(
    order.user,
...
export const updateOrderStatus = async (req, res) => {
  ...
  if (
    !group ||
    !["processing", "shipped", "delivered"].includes(req.body.status) ||
    !transition(group, req.body.status)
  )
    return fail(res, 400, "Select a vendor group and valid next status");
  group.status = req.body.status;
  if (group.status === "shipped") {
    if (!req.body.carrier || !req.body.trackingNumber)
      return fail(res, 400, "Tracking details required");
    group.carrier = String(req.body.carrier).slice(0, 80);
    group.trackingNumber = String(req.body.trackingNumber).slice(0, 100);
    group.shippedAt = new Date();
  }
  ...
  await notify(
    ...
    "order",
    "Fulfillment update",
    `Your order is ${group.status}`,
    "/orders"
  );
  const user = await User.findById(order.user);
  if (user) sendOrderStatusUpdateEmail(user, order, group.status).catch(() => {});
  res.json({ success: true, data: await Order.findById(order._id).populate(populate) });
};
export const cancelVendorOrder = async (req, res) => {
  const order = mongoose.isValidObjectId(req.params.id)
    ? await Order.findById(req.params.id).populate("payment")
    : null;
  if (!order) return fail(res, 404, "Order not found");
  ...
  if (!group || !["confirmed", "pending"].includes(group.status))
    return fail(res, 400, "This vendor order cannot be cancelled");
  ...
  try {
    ...
    group.history.push({ status: "cancelled" });
    order.status = aggregateStatus(order);
    order.statusHistory.push({ status: order.status, note: "Cancellation and refund initiated" });
    await order.save();
    await notify(
      order.user,
      "refund",
      "Cancellation submitted",
      "Refund initiated through Stripe",
      "/orders"
    );
    return res.json({ success: true, data: order });
  } catch (error) {
    ...
    return fail(res, 502, "Unable to process refund");
  }
};
