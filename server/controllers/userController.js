...
import { sendPasswordResetEmail, sendWelcomeEmail } from "../services/notificationService.js";
...
export const createUser = async (req, res, next) => {
  try {
    ...
    if (await User.findOne({ email })) {
      return res
        .status(400)
        .json({ success: false, message: "Email already exists", data: null, error: null });
    }
...
export const loginUser = async (req, res, next) => {
  try {
    ...
    if (!user || !(await bcrypt.compare(result.data.password, user.password))) {
      return res
        .status(401)
        .json({ success: false, message: "Invalid email or password", data: null, error: null });
    }
    if (user.isBlocked) {
...
export const getMe = async (req, res, next) => {
  try {
    ...
    if (!user)
      return res
        .status(404)
        .json({ success: false, message: "User not found", data: null, error: null });
...
export const addAddress = async (req, res, next) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "Address added", data: user.addresses, error: null });
  } catch (error) {
    next(error);
  }
};
...
export const forgotPassword = async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      message:
        "If the email exists, a reset link has been generated. In local development check the server console.",
      data: null,
      error: null,
    });
  } catch (error) {
...
export const updateUserRole = async (req, res, next) => {
  try {
    ...
    if (!["customer", "vendor", "admin"].includes(role))
      return res
        .status(400)
        .json({ success: false, message: "Invalid role", data: null, error: null });
    if (role === "vendor" && !(await Vendor.exists({ user: req.params.id, status: "approved" })))
      return res.status(400).json({
        success: false,
        message: "Approve the seller application before assigning vendor role",
      });
...
export const toggleUserBlock = async (req, res, next) => {
  try {
    if (req.params.id === req.user.userId && req.body.isBlocked)
